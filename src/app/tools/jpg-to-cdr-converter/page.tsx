...
import { ArrowRight, BookOpen, Code2, Eye, FileImage, Layers } from 'lucide-react';
...
export default function Page() {
  return (
    <ExpansionToolPage
      ...
      title="JPG to CDR Converter"
      description="Convert and prepare JPG or JPEG images for CorelDRAW entirely in your browser with zero server uploads. Choose Preserve Mode for photos and layout graphics, or Vectorize Mode to trace solid-color logos into editable SVG vector paths."
    >
      ...
      <div className="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 text-xs text-[var(--muted-foreground)] shadow-xs">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 font-medium text-[var(--foreground)]">
            ...
            <span>100% Client-Side: Lossless JPEG embedding or vector edge tracing with zero server upload</span>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
            ...
            >
              ...
              <span>PNG to CDR (Alpha Transparency)</span>
            </Link>
            <Link
              href="/tools/rgb-cmyk-image-checker"
              className="inline-flex items-center gap-1 rounded-lg border border-[var(--border)] bg-[var(--background)] px-2.5 py-1 text-[var(--foreground)] transition-colors hover:border-indigo-500 hover:text-indigo-600"
            >
              <Layers className="size-3 text-indigo-500" />
              <span>RGB vs CMYK Preflight</span>
            </Link>
            <Link
