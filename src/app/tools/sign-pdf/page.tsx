...
export default function SignPDFTool() {
  ...
  const [posY, setPosY] = useState<number>(150);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        alert("Failed to parse file structure. It may be encrypted.");
        setFile(null);
      }
    }
  };
  ...
  const processSignature = async () => {
    ...
    try {
      // Export canvas drawing as transparent PNG data URL
      ...
      const pngDims = pngImage.scale(scale); // Scale size
      ...
      a.href = url;
      a.download = `Navorika_Signed_${Date.now()}.pdf`;
      document.body.appendChild(a);
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error(error);
      alert("Failed to inject electronic signature stream.");
    }
    ...
    <main className="max-w-4xl mx-auto px-6 py-12 lg:px-8">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold uppercase tracking-wider mb-4 border border-emerald-500/20">
          <ShieldCheck className="h-4 w-4" /> Secure Client Vector Stamping
        </div>
          ...
          <div className="p-8">
            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 mb-8">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 truncate">{file.name} ({pageCount} pages)</span>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-8">
              {/* Signature Pad */}
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2 flex items-center justify-between">
                  <span>Draw Signature</span>
                  <button onClick={clearSignature} className="text-xs text-red-500 hover:text-red-600 flex items-center gap-1"><Eraser className="h-3 w-3"/> Clear</button>
                </label>
                <canvas 
                  ref={canvasRef}
                  width={400}
                  height={200}
                  className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-xl cursor-crosshair touch-none"
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Signature Scale (Size)</label>
                  <input type="range" min={0.1} max={2} step={0.1} value={scale} onChange={(e) => setScale(Number(e.target.value))} className="w-full accent-indigo-600" />
                </div>
              </div>
            </div>

            <div className="flex justify-end border-t border-slate-100 dark:border-slate-800 pt-6">
              <button onClick={processSignature} disabled={isProcessing} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition shadow-md">
                {isProcessing ? <Loader2 className="h-5 w-5 animate-spin" /> : <PenTool className="h-5 w-5" />}
                {isProcessing ? 'Applying Signature...' : 'Sign & Download'}
              </button>
            </div>
          </div>
        )}
