  ...
  const processResize = async () => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          // Disable image smoothing for sharper pixel-art scaling if requested, but default to true
      ...
      img.onerror = () => {
        alert('Failed to load origin image bounds.');
        setIsProcessing(false);
      };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-8 space-y-6">
        {!file ? (
          <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-purple-300 dark:border-purple-500/30 rounded-2xl p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/5 transition-colors">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Upload Image to Resize</h3>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between mb-6">
                <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Dimension Parameters</h4>
                <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-slate-500">
                  <input 
                    type="checkbox" 
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono text-lg outline-none focus:border-purple-500"
                  />
                </div>
                <div>
            ...
            <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
              <button onClick={processResize} disabled={isProcessing} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-xl font-bold transition shadow-md">
                <Maximize className="h-5 w-5" />
                {isProcessing ? 'Rendering...' : 'Resize & Download'}
              </button>
            </div>
          </div>
        )}
