...
function DeletePDFPagesTool() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const pageNumbers = pagesToDelete.split(',').map(p => parseInt(p.trim())).filter(p => !isNaN(p) && p > 0 && p <= totalPages);
          ...
          >
            {!file ? (
              <div>
                ...
                <p className="text-lg font-medium mb-2">Upload PDF to delete pages</p>
          ...
          {file && (
            <div className="flex flex-wrap gap-3">
              ...
              {downloadUrl && (
                <a
                  href={downloadUrl}
                  download={`modified-${file?.name || 'file.pdf'}`}
...
export default function DeletePDFPagesToolWrapper() {
