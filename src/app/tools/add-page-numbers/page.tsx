...
import { FileText, ArrowLeft, Upload, X, ShieldCheck, Download, Loader2, Hash } from 'lucide-react';
...
export default function AddPageNumbersTool() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        alert("Failed to read file arrays. Ensure document is unencrypted.");
        setFile(null);
      }
    }
  };
  ...
  const processPageNumbers = async () => {
    ...
    try {
      ...
      // Embed standard font to avoid external network requests
      ...
      pages.forEach((page, index) => {
        ...
        // Calculate text width to perfectly center it on the X axis
        const textWidth = helveticaFont.widthOfTextAtSize(text, fontSize);
        
        page.drawText(text, {
          x: (width / 2) - (textWidth / 2),
          y: 20, // 20 units from the bottom margin
          size: fontSize,
          font: helveticaFont,
      ...
      a.href = url;
      a.download = `Navorika_Numbered_${Date.now()}.pdf`;
      ...
      alert("Failed to draw strings onto the document array.");
    }
        ...
        {!file ? (
          <div className="p-8">
            ...
            >
              ...
              <p className="text-sm text-slate-500 mt-2">Paginate your files instantly</p>
              <input 
                type="file" 
          ...
          <div className="p-8">
            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 mb-8">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div>
                  ...
                  <span className="text-xs text-slate-400 font-bold block">{pageCount} Pages will be numbered</span>
                </div>
              </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 text-center mb-8">
              <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">
                Numbers will be drawn in standard <span className="font-bold text-slate-900 dark:text-white">Helvetica</span> font at the <span className="font-bold text-slate-900 dark:text-white">bottom center</span> of every page using the format:
              </p>
              <div className="mt-4 px-4 py-2 inline-block bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm rounded-lg text-sm font-mono text-slate-700 dark:text-slate-300">
                Page 1 of {pageCount}
              </div>
            </div>
            ...
            <div className="flex justify-end border-t border-slate-100 dark:border-slate-800 pt-6">
              ...
              >
                {isProcessing ? <Loader2 className="h-5 w-5 animate-spin" /> : <Hash className="h-5 w-5" />}
                {isProcessing ? 'Stamping Numbers...' : 'Add Numbers & Download'}
              </button>
            </div>
          </div>
        )}
