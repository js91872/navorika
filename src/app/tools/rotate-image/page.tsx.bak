...
export default function RotateImageTool() {
  ...
  const applyTransformations = (img: HTMLImageElement) => {
    const canvas = canvasRef.current;
    const ctx = canvas?.getContext('2d');
    if (!ctx || !canvas) return;

    // Calculate new canvas dimensions if rotated 90 or 270 degrees
    ...
    // Move to canvas center for rotation/flip origin
    ...
    // Draw image offset by half its intrinsic width/height
    ...
    <main className="max-w-5xl mx-auto px-6 py-12 lg:px-8">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 text-purple-600 dark:text-purple-400 text-xs font-bold uppercase tracking-wider mb-4 border border-purple-500/20">
          <ShieldCheck className="h-4 w-4" /> Canvas Coordinate Matrix
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden p-8 grid md:grid-cols-3 gap-8">
        ...
        <div className="space-y-6 flex flex-col justify-between h-full">
          {!file ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-3">
                <button onClick={() => setRotation(r => r - 90)} className="flex flex-col items-center justify-center gap-2 p-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-purple-500 hover:text-purple-600 transition-colors">
                  <RotateCcw className="h-5 w-5" />
                  <span className="text-[10px] font-bold uppercase">Left 90°</span>
                </button>
                <button onClick={() => setRotation(r => r + 90)} className="flex flex-col items-center justify-center gap-2 p-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-purple-500 hover:text-purple-600 transition-colors">
                  <RotateCw className="h-5 w-5" />
                  <span className="text-[10px] font-bold uppercase">Right 90°</span>
                </button>
                <button onClick={() => setFlipH(!flipH)} className="flex flex-col items-center justify-center gap-2 p-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-purple-500 hover:text-purple-600 transition-colors">
                  <FlipHorizontal className="h-5 w-5" />
                  <span className="text-[10px] font-bold uppercase">Flip H</span>
                </button>
                <button onClick={() => setFlipV(!flipV)} className="flex flex-col items-center justify-center gap-2 p-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-purple-500 hover:text-purple-600 transition-colors">
          ...
          {file && (
            <button onClick={downloadTransformedImage} disabled={isProcessing} className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white py-3 rounded-xl font-bold transition shadow-md">
        ...
        <div className="md:col-span-2 border border-slate-100 dark:border-slate-800 rounded-2xl bg-slate-50 dark:bg-slate-950 p-4 flex items-center justify-center min-h-[300px] overflow-hidden">
          {file ? (
            <canvas ref={canvasRef} className="max-w-full max-h-[400px] object-contain rounded shadow-md transition-transform" />
          ) : (
