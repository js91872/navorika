...
export default function TaxationDeckTool() {
  ...
  const [valB, setValB] = useState<number | ''>(18);
  const [valC, setValC] = useState<number | ''>(20000);
  const [result, setResult] = useState<any>(null);

  const configs: Record<string, any> = {
    'gst-calculator': { labelA: 'Base Amount (₹)', labelB: 'GST Rate (%)', showC: false,
      calc: (a: number, b: number) => {
        const gstAmount = a * (b / 100);
        return { value: (a + gstAmount).toLocaleString('en-IN'), unit: `₹ Total (Includes ₹${gstAmount.toLocaleString('en-IN')} GST)` };
      }},
    'hra-calculator': { labelA: 'Basic Salary (Yearly ₹)', labelB: 'HRA Received (Yearly ₹)', labelC: 'Actual Rent Paid (Yearly ₹)',
      calc: (a: number, b: number, c: number) => {
        const rentMinus10 = c - (0.10 * a);
        const halfBasic = a * 0.50;
        const exemption = Math.max(0, Math.min(b, rentMinus10, halfBasic));
    ...
    'income-tax-calculator': { labelA: 'Total Annual Income (₹)', labelB: 'Section 80C/Deductions (₹)', showC: false,
      calc: (a: number, b: number) => {
        let taxable = Math.max(0, a - b - 50000);
        let tax = 0;
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="bg-white dark:bg-slate-900 border rounded-3xl p-8 space-y-4 shadow-sm">
          <div>
            ...
            <input type="number" value={valA} onChange={e=>setValA(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border font-bold outline-none" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">{config.labelB}</label>
            <input type="number" value={valB} onChange={e=>setValB(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border font-bold outline-none" />
          </div>
          {config.showC !== false && <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">{config.labelC}</label>
            <input type="number" value={valC} onChange={e=>setValC(e.target.value === '' ? '' : Number(e.target.value))} className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border font-bold outline-none" />
          </div>}
        </div>
