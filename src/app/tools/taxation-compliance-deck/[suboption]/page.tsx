...
export default function DynamicSubOptionTool() {
  ...
  const suboption = (params?.suboption as string) || 'income-tax-calculator';
  
  const seo = taxSubTools[suboption] || taxSubTools['income-tax-calculator'];
  ...
  const jsonLdSchema = {
    ...
    "url": `https://navorika.com/tools/taxation-compliance-deck/${suboption}`,
    ...
    <main className="max-w-6xl mx-auto px-6 py-12 lg:px-8">
      ...
      <div className="flex flex-wrap gap-2 mb-8 bg-slate-100 dark:bg-slate-950 p-1.5 rounded-2xl w-fit border">
        {Object.keys(taxSubTools).map((key) => (
          <button 
            key={key} 
            onClick={() => router.push(`/tools/taxation-compliance-deck/${key}`)}
