    ...
    <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div><label className="text-xs font-bold uppercase text-slate-600 block mb-2">Total Card Balance</label><input type="number" value={ccBalance} onChange={(e) => setCcBalance(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-semibold text-slate-900 focus:outline-none focus:border-indigo-500" /></div>
          <div><label className="text-xs font-bold uppercase text-slate-600 block mb-2">Annual Interest Rate (APR %)</label><input type="number" step="0.5" value={ccInterestRate} onChange={(e) => setCcInterestRate(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-semibold text-slate-900 focus:outline-none focus:border-indigo-500" /></div>
          <div><label className="text-xs font-bold uppercase text-slate-600 block mb-2">Planned Monthly Payment</label><input type="number" value={ccMonthlyPayment} onChange={(e) => setCcMonthlyPayment(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-semibold text-slate-900 focus:outline-none focus:border-indigo-500" /></div>
        </div>
