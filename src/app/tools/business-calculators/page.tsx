  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6">
      <div className="mb-8 border-b border-slate-200 dark:border-slate-800 dark:border-slate-800 pb-6 transition-colors">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white dark:text-white tracking-tight">Business Finance Hub</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-900 transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6 transition-colors">
          <h3 className="font-bold text-slate-900 dark:text-white dark:text-white text-base">Adjust Parameters</h3>

          {mode === 'profit_margin' && (
            <>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Total Revenue (Sales)</label><input type="number" value={revenue} onChange={(e) => setRevenue(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Cost of Goods Sold (COGS)</label><input type="number" value={cogs} onChange={(e) => setCogs(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Operating Expenses (Fixed)</label><input type="number" value={opEx} onChange={(e) => setOpEx(e.target.value)} className={inputClassName} /></div>
            </>
          )}

          {mode === 'pricing' && (
            <>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Base Cost Price</label><input type="number" value={costPrice} onChange={(e) => setCostPrice(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Markup Percentage (%)</label><input type="number" value={markupPercent} onChange={(e) => setMarkupPercent(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Discount Offered (%)</label><input type="number" value={discountPercent} onChange={(e) => setDiscountPercent(e.target.value)} className={inputClassName} /></div>
            </>
          )}

          {mode === 'sales_billing' && (
            <>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Base Invoice Amount</label><input type="number" value={baseInvoice} onChange={(e) => setBaseInvoice(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Applicable Tax Rate (%)</label><input type="number" value={taxPercent} onChange={(e) => setTaxPercent(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Sales Commission Rate (%)</label><input type="number" value={commissionPercent} onChange={(e) => setCommissionPercent(e.target.value)} className={inputClassName} /></div>
            </>
          )}
          ...
          {mode === 'viability' && (
            <div className="space-y-4">
              ...
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Total Fixed Costs</label><input type="number" value={fixedCosts} onChange={(e) => setFixedCosts(e.target.value)} className={inputClassName} /></div>
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800"><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Annual EBITDA</label><input type="number" value={annualEbitda} onChange={(e) => setAnnualEbitda(e.target.value)} className={inputClassName} /></div>
              <div><label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 dark:text-slate-400 block mb-2">Valuation Multiple (x)</label><input type="number" step="0.5" value={valuationMultiple} onChange={(e) => setValuationMultiple(e.target.value)} className={inputClassName} /></div>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-slate-900 transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl p-6 shadow-sm transition-colors">
              ...
              <div className={`text-3xl font-black ${calculations.val1 < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-white dark:text-white'}`}>{calculations.isCurrency1 ? formatCurrency(calculations.val1) : `${calculations.val1.toLocaleString('en-IN')}${mode === 'profit_margin' ? '%' : ''}`}</div>
            </div>
            <div className="bg-white dark:bg-slate-900 transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl p-6 shadow-sm transition-colors">
            ...
            <div className="bg-white dark:bg-slate-900 transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl p-6 shadow-sm transition-colors">
              ...
              <div className={`text-3xl font-black ${calculations.val3 < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-white dark:text-white'}`}>{calculations.isCurrency3 ? formatCurrency(calculations.val3) : calculations.val3}</div>
            </div>
            <div className="bg-white dark:bg-slate-900 transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl p-6 shadow-sm transition-colors">
          ...
          <div className="bg-slate-50 dark:bg-slate-950 transition-colors dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl p-6 shadow-inner text-slate-700 dark:text-slate-300 dark:text-slate-300 transition-colors">
            <div className="text-sm font-semibold leading-relaxed">{calculations.summary}</div>
          </div>

          <div className="p-6 bg-white dark:bg-slate-900 transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-800 dark:border-slate-800 rounded-2xl text-xs text-slate-500 dark:text-slate-400 shadow-sm transition-colors">
            <h4 className="font-bold text-slate-700 dark:text-slate-300 dark:text-slate-300 mb-2">Calculators Fully Supported in this Suite:</h4>
