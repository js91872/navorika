...
export default function PDFSecurityHub() {
  ...
  const inputClassName = "w-full bg-white dark:bg-slate-950 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm font-semibold focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-500 transition-colors";

  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6">
      <div className="mb-8 border-b border-slate-200 dark:border-slate-800 pb-6 transition-colors">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">PDF Security & Permissions</h1>
      ...
      <div className="flex border border-slate-200 dark:border-slate-800 rounded-xl p-1 bg-white dark:bg-slate-900 max-w-xs mb-8 shadow-sm transition-colors">
        <button
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 transition-colors">
        <div>
          <label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block mb-3">Upload PDF File</label>
          <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-6 text-center bg-slate-50 dark:bg-slate-950/50 hover:bg-slate-100/60 dark:hover:bg-slate-900 transition-colors relative cursor-pointer">
            <input type="file" accept=".pdf" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
            <div className="text-2xl mb-1">{mode === 'lock' ? '🔒' : '🔓'}</div>
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{selectedFile ? `Selected: ${selectedFile.name}` : 'Select document file to process'}</p>
          </div>
        </div>

        <div className="p-6 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl space-y-4 transition-colors">
          <h3 className="font-bold text-slate-900 dark:text-white text-sm border-b border-slate-200 dark:border-slate-800 pb-2">Security Parameters</h3>

          {mode === 'lock' ? (
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block mb-2">Set Document Password</label>
              ...
              <div className="space-y-2 pt-2">
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="print-check" className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Restrict Printing Privileges</label>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="edit-check" className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Restrict Content Copying & Editing</label>
                </div>
              </div>
            </div>
          ) : (
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block mb-2">Enter Current Password</label>
