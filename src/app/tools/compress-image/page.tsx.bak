...
export default function CompressImageTool() {
  ...
  const [quality, setQuality] = useState(0.6); // Default 60% quality
    ...
    try {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          // Determine optimal output format (keep WEBP/JPEG, force PNG to WEBP for compression)
          let mimeType = file.type;
          if (mimeType === 'image/png') mimeType = 'image/webp'; // PNG compression requires format shift
          if (!['image/jpeg', 'image/webp'].includes(mimeType)) mimeType = 'image/jpeg';
          ...
          canvas.toBlob((blob) => {
            if (blob) {
              ...
              a.href = downloadUrl;
              a.download = `Navorika_Compressed_${file.name.split('.')[0]}.${mimeType.split('/')[1]}`;
      ...
      img.onerror = () => {
        alert('Failed to load image into compression canvas.');
        setIsProcessing(false);
      };
    ...
    <main className="max-w-4xl mx-auto px-6 py-12 lg:px-8">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 text-purple-600 dark:text-purple-400 text-xs font-bold uppercase tracking-wider mb-4 border border-purple-500/20">
          <ShieldCheck className="h-4 w-4" /> Local Quantization Loop
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-8 space-y-6">
        {!file ? (
          <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-purple-300 dark:border-purple-500/30 rounded-2xl p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/5 transition-colors">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Upload Image to Compress</h3>
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <span className="text-xs font-mono text-slate-400 bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded">{(file.size / 1024 / 1024).toFixed(2)} MB</span>
              </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
              <div className="flex justify-between items-center mb-4">
                <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Compression Target Quality</label>
