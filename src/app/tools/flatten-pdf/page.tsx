...
export default function FlattenPDFTool() {
  ...
  const processFlattening = async () => {
    ...
    try {
      ...
      if (!form) {
        alert("No interactive form fields found in this document.");
      } else {
        // Stamp all fields into standard layout graphics
        form.flatten();
      }
      ...
      a.href = url;
      a.download = `Navorika_Flattened_${file.name}`;
      document.body.appendChild(a);
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error(error);
      alert("Failed to flatten interactive structural arrays.");
    }
    ...
    <main className="max-w-4xl mx-auto px-6 py-12 lg:px-8">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold uppercase tracking-wider mb-4 border border-emerald-500/20">
          <ShieldCheck className="h-4 w-4" /> AcroForm Object Compiler
        </div>
        ...
        {!file ? (
          <div className="p-8">
            <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-indigo-300 dark:border-indigo-500/30 rounded-2xl p-12 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-500/5 transition-colors">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-white">Select Fillable PDF Form</h3>
          ...
          <div className="p-8">
            ...
            <div className="flex justify-end border-t border-slate-100 dark:border-slate-800 pt-6">
              <button onClick={processFlattening} disabled={isProcessing} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition shadow-md">
                {isProcessing ? <Loader2 className="h-5 w-5 animate-spin" /> : <Layers className="h-5 w-5" />}
                {isProcessing ? 'Converting Layouts...' : 'Flatten Form & Download'}
              </button>
            </div>
          </div>
        )}
