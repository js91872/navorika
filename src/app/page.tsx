...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      {/* Hero Section */}
      <section className="relative overflow-hidden pt-20 pb-24 lg:pt-32 lg:pb-36 border-b border-slate-200 bg-gradient-to-b from-white via-slate-50 to-slate-100/50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-xs font-semibold mb-8 tracking-wide shadow-sm">
            <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
            Trusted by 10,000+ active daily users
          </div>
          
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tight text-slate-900 max-w-4xl mx-auto leading-[1.1] mb-6">
          ...
          <div className="max-w-xl mx-auto mb-12">
            <div className="relative flex items-center shadow-xl shadow-indigo-500/5 rounded-2xl p-1 bg-white border border-slate-200">
              ...
              <input 
                ...
                className="w-full bg-transparent border-none px-3 py-3 text-slate-900 placeholder-slate-400 focus:outline-none text-sm sm:text-base"
              />
          ...
          {/* Quick Metrics Bar */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto pt-8 border-t border-slate-200 text-left">
            <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
              <div className="text-2xl font-black text-slate-900">40+</div>
              <div className="text-xs text-slate-500 font-medium mt-0.5">Calculators & Tools</div>
            </div>
            <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
              <div className="text-2xl font-black text-slate-900">15+</div>
              <div className="text-xs text-slate-500 font-medium mt-0.5">PDF Utilities</div>
            </div>
            <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
              <div className="text-2xl font-black text-slate-900">100%</div>
              <div className="text-xs text-slate-500 font-medium mt-0.5">Client-Side Private</div>
            </div>
            <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm">
              <div className="text-2xl font-black text-slate-900">10K+</div>
              <div className="text-xs text-slate-500 font-medium mt-0.5">Active Users</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="categories" className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
          <div>
            <span className="text-indigo-600 text-xs font-bold uppercase tracking-widest block mb-2">Specialized Collections</span>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">Explore Our Categories</h2>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((cat, idx) => (
            <div key={idx} className="group relative p-8 rounded-2xl bg-white border border-slate-200 hover:border-indigo-300 transition-all duration-300 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 flex flex-col justify-between overflow-hidden">
              ...
              <div>
                <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center mb-6 shadow-sm ${cat.iconBg}`}>
                  {cat.svg}
                </div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-xl font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{cat.name}</h3>
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">{cat.count}</span>
                </div>
      ...
      <section className="py-24 border-t border-slate-200 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-indigo-600 text-xs font-bold uppercase tracking-widest block mb-2">Why Choose Navorika</span>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight mb-4">Engineered for Excellence</h2>
          <p className="text-slate-600 text-base">We combine uncompromising speed with absolute data privacy.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm">
            ...
            <p className="text-sm text-slate-600 leading-relaxed">Instant calculations and transformations running entirely in your browser with zero latency delays.</p>
          </div>
          <div className="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm">
            <div className="w-12 h-12 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-center justify-center mb-6">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
              </svg>
            </div>
            <h3 className="text-lg font-bold text-slate-900 mb-2">100% Private & Secure</h3>
            <p className="text-sm text-slate-600 leading-relaxed">Your files and calculations never leave your device. Complete local execution ensures absolute confidentiality.</p>
          </div>
          <div className="p-8 rounded-2xl bg-white border border-slate-200 shadow-sm">
            <div className="w-12 h-12 rounded-xl bg-violet-50 border border-violet-100 text-violet-600 flex items-center justify-center mb-6">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
              </svg>
            </div>
            <h3 className="text-lg font-bold text-slate-900 mb-2">Always Free</h3>
            <p className="text-sm text-slate-600 leading-relaxed">No subscriptions, no hidden walls, and no mandatory registration. Access everything instantly.</p>
          </div>
        </div>
      </section>
