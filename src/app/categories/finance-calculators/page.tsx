...
import { ArrowLeft, FileText, Calculator, ArrowRight } from 'lucide-react';
import { categories, tools } from '@/data/registry';

export default function FinanceCategoryPage() {
  const category = categories.find((c) => c.slug === 'finance-calculators')!;
  const categoryTools = tools.filter((t) => t.category === 'finance-calculators');
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 rounded-lg bg-gradient-to-br from-blue-600 to-cyan-600 text-white shadow-sm">
          ...
          >
            <div>
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                  <Calculator className="h-5 w-5" />
                </div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{tool.title}</h2>
              </div>
            ...
            <div className="flex justify-end">
              <div className="h-8 w-8 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 transition-colors">
