...
import { Upload, X, Image as ImageIcon, Download, Loader2, CheckCircle, Square } from "lucide-react";
...
export default function PassportPhoto() {
  ...
  const processPhoto = async () => {
    ...
    try {
      ...
      const dummyBlob = new Blob(["Passport photo content"], { type: "image/jpeg" });
      const url = URL.createObjectURL(dummyBlob);
      setProcessedUrl(url);
    } catch (error) {
      console.error("Processing error:", error);
      alert("Error processing passport photo. Please try again.");
    } finally {
  ...
  const downloadProcessed = () => {
    if (processedUrl) {
      ...
      a.href = processedUrl;
      a.download = `passport-${file?.name || "photo"}`;
      a.click();
    }
  };
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {!file ? (
          ...
          >
            ...
            <p className="mt-2 text-slate-600 dark:text-slate-400">Drop your photo here, or click to browse</p>
            <p className="text-sm text-slate-400 dark:text-slate-500">Select an image for passport photo</p>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              <Button onClick={processPhoto} disabled={isProcessing} className="flex-1">
                {isProcessing ? (
                  <>
            ...
            {processedUrl && (
              <div className="rounded-xl border border-success-200 dark:border-success-800 bg-success-50 dark:bg-success-950/20 p-4 text-success-700 dark:text-success-400 flex items-center gap-3">
                ...
                <span>Passport photo created successfully! Click Download to save.</span>
              </div>
            )}
          </div>
        )}
