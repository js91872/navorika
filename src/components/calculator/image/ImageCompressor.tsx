...
export default function ImageCompressor() {
  ...
  const compressImage = async () => {
    ...
    try {
      ...
      const compressedSize = originalSize * 0.4; // Simulate 60% compression
      setCompressionRatio(60);
      
      // Create a dummy blob
      const dummyBlob = new Blob(["Compressed image content"], { type: file.type });
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {!file ? (
          ...
          >
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500">Select an image to compress</p>
          ...
          <div className="space-y-4">
            ...
            {compressedUrl && compressionRatio && (
              <div className="rounded-xl border border-success-200 dark:border-success-800 bg-success-50 dark:bg-success-950/20 p-4 text-success-700 dark:text-success-400">
                <div className="flex items-center gap-3">
                  <CheckCircle className="h-5 w-5" />
                  <span>Image compressed successfully! ({compressionRatio}% smaller)</span>
                </div>
                <p className="text-sm text-success-600 dark:text-success-300 mt-1 ml-8">
