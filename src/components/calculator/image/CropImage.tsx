...
import { Upload, X, Image as ImageIcon, Download, Loader2, CheckCircle, Crop } from "lucide-react";
...
export default function CropImage() {
  ...
  const cropImage = async () => {
    ...
    try {
      ...
      const dummyBlob = new Blob(["Cropped image content"], { type: file.type });
  ...
  const downloadCropped = () => {
    if (croppedUrl) {
      ...
      a.href = croppedUrl;
      a.download = `cropped-${file?.name || "image"}`;
      a.click();
    }
  };
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {!file ? (
          ...
          >
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500">Select an image to crop</p>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              <Button onClick={cropImage} disabled={isCropping} className="flex-1">
                {isCropping ? (
                  <>
              ...
              {croppedUrl && (
                <Button variant="outline" onClick={downloadCropped}>
            ...
            {croppedUrl && (
              <div className="rounded-xl border border-success-200 dark:border-success-800 bg-success-50 dark:bg-success-950/20 p-4 text-success-700 dark:text-success-400 flex items-center gap-3">
                ...
                <span>Image cropped successfully! Click Download to save.</span>
              </div>
            )}
          </div>
        )}
