...
export default function ExpansionToolPage({ category, eyebrow, title, description, slug, children }: { category: 'developer-tools' | 'construction-calculators' | 'finance-calculators' | 'everyday-calculators' | 'health-calculators'; eyebrow: string; title: string; description: string; slug?: string; children: React.ReactNode }) {
  const categoryName = category === 'developer-tools' ? 'Developer Tools' : category === 'finance-calculators' ? 'Finance Calculators' : category === 'everyday-calculators' ? 'Everyday Calculators' : category === 'health-calculators' ? 'Health Calculators' : 'Construction Calculators';
